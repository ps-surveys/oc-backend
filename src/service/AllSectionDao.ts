...
class ActSectionDao {
  ...
  public async sectionsNotInForm(idform): Promise<any> {
    try {
      var rsw: any = await pool.query(Query.SELECT_SECTIONS_NOT_IN_FORM, [
        idform,
      ]);
      const rs = rsw.rows;
      var list = [];
  ...
  public async insert(section: ActSection): Promise<any> {
    ...
    // var id_rf: number;
    try {
      const sec = await pool.query(Query.INSERT_SECTION, [
        section.nameSection,
        section.descSection,
        section.typeSection,
        section.tableName,
      ...
      create_table_sections =
        "CREATE TABLE ACT_SECTION_" +
        id_sec +
        " (ID_ACST SERIAL not null, ID_SECTION INT4 null, ID_RF INT4 null, CREATION_DATE TIMESTAMP WITH TIME ZONE null default CURRENT_TIMESTAMP, constraint PK_ACT_SECTION_" +
        id_sec +
        " primary key (ID_ACST));";
      await pool.query(create_table_sections, null);

      create_index_sections =
        "CREATE INDEX ACT_SECTION_ACT_SECTION_" +
        id_sec +
        "_FK ON ACT_SECTION_" +
        id_sec +
        " (ID_SECTION);";
      ...
      create_index_regformat =
        "CREATE INDEX ACT_REGISTER_FORMAT_ACT_SECTION_" +
        id_sec +
        "_FK ON ACT_SECTION_" +
        id_sec +
        " (ID_RF);";
